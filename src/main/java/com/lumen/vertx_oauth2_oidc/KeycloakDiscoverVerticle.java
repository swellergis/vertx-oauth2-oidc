    ...
    public void start(Promise<Void> startPromise) throws Exception 
    {
        ...
        vertx.createHttpServer(options)
            ...
            .onSuccess(server -> {
                ...
                LOG.log(Level.INFO, String.format("HTTP server started on port %s", server.actualPort()));
            })
            .onFailure(event -> {
                startPromise.fail(event);
                LOG.log(Level.SEVERE, String.format("Failed to start HTTP server:%s", event.getMessage()));
            }
        );
