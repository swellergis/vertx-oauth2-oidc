...
class ProtectedProfileHandler implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext ctx) {
        authProvider.userInfo(ctx.user()).onFailure(err -> {
            ...
            WebClient.create(ctx.vertx()).getAbs("https://api.github.com/user/emails")
                .authentication(new TokenCredentials(
                    ctx.user().<String>get("access_token")))
