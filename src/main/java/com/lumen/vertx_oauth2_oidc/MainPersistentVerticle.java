    ...
    public void start(Promise<Void> startPromise) {
        ...
        router.get("/protected")
                        .handler(OAuth2AuthHandler
                                        ...
                                        .withScope("user:email").pkceVerifierLength(64))
                        .handler(ctx -> {
                            authProvider.userInfo(ctx.user()).onFailure(err -> {
                                ctx.session().destroy();
                                ctx.fail(err);
                            }).onSuccess(userInfo -> {
                                ...
                                WebClient.create(ctx.vertx())
                                                ...
                                                .authentication(new TokenCredentials(ctx.user()
        ...
        vertx.createHttpServer().requestHandler(router).listen(Integer.getInteger("port", 8080))
                        .onSuccess(server -> {
                            System.out.println(
